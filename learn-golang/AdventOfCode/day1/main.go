...
func main()  {
  conteudo, errors := os.Open("t.txt")
  if errors != nil {
    log.Fatal(errors)
  }
  defer conteudo.Close()

  linha := bufio.NewScanner(conteudo)

  for linha.Scan() {
    fmt.Println(linha.Text())
    for _, letra := range linha.Text() {
      fmt.Println(string(letra))
          }
  }
  // caminho_arquivo := "C:\\Users\\Samsung\\Projetos\\Learn-Golang\\AdventOfCode\\day1\\info.txt"
  // infos, erros := os.Open(caminho_arquivo)
  // if erros != nil {
  ...
  // scanner := bufio.NewScanner(infos)
